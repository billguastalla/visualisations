...
public:
	VectorField(int arrowRes = 5, float cellSize = 5.0f, float drawDistance = 100.0f, float arrowSize = 0.2f)
		:
		m_arrow{},
		m_arrowShader{ new Shader{"../../assets/shaders/Cubes_Vertex.vs","../../assets/shaders/Cubes_ObjectFragment.fs"} },
		m_cellSize{ cellSize },
		...
		for (float x{ 0.0f }; x < m_drawDistance; x += m_cellSize)
			for (float y{ 0.0f }; y < m_drawDistance; y += m_cellSize)
				for (float z{ 0.0f }; z < m_drawDistance; z += m_cellSize)
				{
					...
					glm::mat4 m{ 1.0f };
					/*m = glm::rotate(m, x/90.0f, glm::vec3{ 1,0,0 });
					m = glm::scale(m, glm::vec3{
						2.0f*x / m_drawDistance,
						2.0f * y / m_drawDistance,
						2.0f * z / m_drawDistance
						});*/
